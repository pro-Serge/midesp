...
При отправке третьего байта со значением 11h меняется режим? Ответы от машинки начинают полностью повторять отправляемые пакеты.
Если отправить третий байт с любым другим значением, где первый бит равен 1, возвращаемые машинкой пакеты опять содержат четвертый байт равный 2 и третий байт с вариациями на тему 09h, 89h, 99h и т.д.

Параллельно для экспериментов был также приобретен оригинальный WiFi модуль Midea MWB-S-WB01. Электрически (распиновка разъема и уровни сигналов) он совместим с разъемом на плате ПММ, но совместимости протоколов нет. Модуль MWB-S-WB01 использует известный протокол, который Midea применяет в кондиционерах и другой технике.
